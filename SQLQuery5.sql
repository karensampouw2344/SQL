...
SELECT 
    ...
    TicketQuantity * MoviePrice AS TotalMovieSold
FROM 
    Transactions
WHERE 
    DATEPART(QUARTER, TransactionDate) = 1
HAVING 
    TicketQuantity * MoviePrice > (SELECT AVG(TicketQuantity * MoviePrice) FROM Transactions);
-- no. 8
SELECT 
    UPPER(CONCAT('Transaction ', SUBSTRING(TransactionID, 3, LEN(TransactionID) - 2))) AS Transaction,
    StaffName,
    CONCAT('Ms/Mrs. ', CustomerName) AS CustomerName,
    YEAR(TransactionDate) AS TransactionDate
FROM 
    Transactions
WHERE 
    CustomerGender = 'Female'
    AND (SELECT SUM(TransactionAmount) FROM Transactions WHERE CustomerID = Transactions.CustomerID) > 
        (SELECT AVG(TransactionAmount) FROM Transactions);
